...
public class InterfaceXML extends JFrame implements MouseListener{
	...
	//comprueba que una cadena sea alfanumerica
	...
	private void mostrarInfo() {
		...
		try {
			...
			for(int i=0;i<obHijo.getLength();i++) {
				mostrar+="\n-----------------------------\n"+modelo.getNombre()+"\n";
				Node n=obHijo.item(i).getFirstChild();
				int cont=0;
				while(n!=null) {
					mostrar+=modelo.getEtNieto().get(cont);
					mostrar+=n.getTextContent()+"\n";
	...
	//pasa la informacion del auxiliar al fichero
	private void saveOnFile() {
			...
			try {
				...
				for(int i=0;i<aux.getCantNietos();i++) {
					Element e=document.createElement(aux.getEtNieto().get(i));
					e.appendChild(document.createTextNode(aux.getRelleno().get(i)));
					child.appendChild(e);
				}
	...
	private void leerInfo() {
		try {
			Document domDocument =DocumentBuilderFactory.newInstance().newDocumentBuilder().parse(ficherofocus.getAbsolutePath());
			modelo=new Objeto(domDocument.getDocumentElement().getFirstChild().getNodeName());
			ArrayList<String>etiquetasHijo=new ArrayList<String>();
			Node n=domDocument.getDocumentElement().getFirstChild().getFirstChild();
			if(n!=null) {
				etiquetasHijo.add(n.getNodeName());
				n=n.getNextSibling();
				while(n!=null) {
					etiquetasHijo.add(n.getNodeName());
					n=n.getNextSibling();
			...
			modelo.setEtNieto(etiquetasHijo);
		} catch (Exception e) {
			e.printStackTrace();
		} 
	}
